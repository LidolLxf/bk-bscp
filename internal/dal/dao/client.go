...
func (dao *clientDao) UpdateRetriedClientsStatusWithTx(kit *kit.Kit, tx *gen.QueryTx, ids []uint32, all bool) error {
	...
	if len(ids) > 1 {
		...
		for i := 0; i < len(clients) && i < 3; i++ {
			uids = append(uids, clients[i].Attachment.UID)
		}
		ad := dao.auditDao.Decorator(kit, clients[0].Attachment.BizID, &table.AuditField{
			ResourceInstance: fmt.Sprintf(constant.OperateObject+constant.ResSeparator+constant.ConfigRetryClientUID,
				len(clients), strings.Join(uids, constant.NameSeparator)),
			Status: enumor.Success,
			AppId:  clients[0].Attachment.AppID,
